...
bool CFIPEX::sendCommand(byte commandId,byte length, byte data[])
{
	...
	retour=m_serialPort->write(trame,0,length+4);
...
bool CFIPEX::ping(fipexError &error)	//testée et vérifiée le 15/01/15
{
	...
	if(sendCommand((byte)SU_PING,0,NULL))
	{
...
bool CFIPEX::setParameters(byte data[],byte length,fipexError &error)
{
	bool retour=false;
	byte reponse[205]={0};
	byte rspId;
...
bool CFIPEX::getScienceDataPacket(scienceDataPacket *recupSDP)
{
	...
	if(sendCommand((byte)SU_DP,0,NULL))
	{
		if(getReponse(rspId,reponse,length))
		{
			if(rspId==SU_R_SDP)
