...
CScript::CScript(char *standardCheckOut,int taille,typeScript type)
{
	m_position=0;
	m_taille=taille;
	m_type=type;
...
bool CScript::getCommande(byte &idCommand,byte data[], byte &taille, byte delay[2])
{
	bool retour=false;
	if(m_type==binaire)
...
bool  CScript::getCommandeBinaire(byte &idCommand,byte data[], byte &taille, byte delay[2])
{
	...
	bool retour=false;
		...
		if(m_posDebutCommande[m_position]==0x7E)
		{
			...
			m_position+=taille;
			byte checksum=m_posDebutCommande[++m_position];
...
bool CScript::getCommandeASCII(byte &commandId, byte data[], byte &taille, byte delay[2])
{
	bool retour=false;
	/*byte test=0;
	int k=0,m=0;
